---
title: "Interactive Quality Improvement Charts"
subtitle: "An implementation interactive of statistical process control charts for R"
author: "Jacob Anhøj and Seamus McKinsey"
date: "`r Sys.Date()`"
output: 
  rmarkdown::html_vignette:
    ...
    toc_depth: 3
vignette: >
  %\VignetteIndexEntry{Interactive Vignette}
...
The `qicharts2` package contains two main functions for analysis and visualisation of data for continuous quality improvement: `qic.ggiraph()` and `paretochart()`.

`qic()` provides a simple interface for constructing statistical process control (SPC) charts for time series data. `qic.ggiraph()` adds interactivity to these charts through the [ggiraph](https://github.com/davidgohel/ggiraph) package. This vignette demonstrates the new interactive functionality with the same charts as the original qicharts2 vignette. The interactivity has not yet been extended to `paretochart()`.
...
Non-random variation found by the Anhøj rules is visualised by `qic.ggiraph()` using a red and dashed centre line.
...
o <- qic.ggiraph(y, chart = 'i')

# Print summary data frame
summary(o)
```

Many additional tests for special cause variation have been proposed since Shewhart invented the 3-sigma rule, and many tests are currently being used. However, the more tests that are applied to the analysis, the higher the risk of false signals. `qicharts2` takes a conservative approach by applying only three tests, the 3-sigma rule and the two Anhøj rules. That means that a signal from `qic.ggiraph()` most likely represents a significant change in the underlying process.

## Choosing the right control chart

By default, `qic.ggiraph()` produces run charts. To make a control chart, the `chart` argument must be specified.
...
`qic.ggiraph()` includes eleven control charts.
...
Using month as the x axis variable, `qic.ggiraph()` automatically calculates the average age per month.

```{r}
qic.ggiraph(month, age,
    data  = cabg,
    chart = 'xbar',
...
```{r}
qic.ggiraph(month, readmissions, n,
    data      = cabg_by_month,
    chart     = 'p',
    y.percent = TRUE,
...
qic.ggiraph(month, readmissions, 
    n         = n,
    data      = cabg_by_month,
    chart     = 'p',
    y.percent = TRUE,
...
`qic.ggiraph()` from `qicharts2` provides a simple interface for constructing statistical process control charts for time series data. 

The default chart is a run chart, which tests for non-random variation in data over time using the Anhøj rules for unusually long runs and unusually few crossings. The Anhøj rules are most useful for detection of moderate, persistent shifts. To detect larger, possibly transient, shifts, control charts are useful. `qic.ggiraph()` employs 11 types of control charts for different types of data.
