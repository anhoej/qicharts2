...
plot.qic.ggiraph <- function(x, title, ylab, xlab, subtitle, caption, part.labels,
  ...
  if (getOption('qic.clshade')) {
    ...
    p <- p +
      geom_line_interactive(aes_(y = ~ lcl), colour = col1, na.rm = T)

    p <- p +
      geom_line_interactive(aes_(y = ~ ucl), colour = col1, na.rm = T)
  }

  p <- p +
    geom_line_interactive(aes_(y = ~ target), colour = col4, na.rm = T)

  p <- p +
    geom_line_interactive(aes_(y = ~ cl, linetype = ~ runs.signal, colour = ~ linecol),
              na.rm = TRUE) +
  ...
  if (dots.only) {
    p <- p +
      geom_point_interactive(aes_(colour = ~ dotcol, tooltip = ~ lab.format(y, decimals, y.percent)), size = 3 * point.size, na.rm = TRUE)
  } else {
    p <- p +
      geom_line_interactive(colour = col2, size = 1.1, na.rm = TRUE) +
      geom_point_interactive(aes_(colour = ~ dotcol, tooltip = ~ lab.format(y, decimals, y.percent)), size = point.size, na.rm = TRUE)
  }
  ...
  if (show.labels) {
    p <- p +
      geom_text_interactive(aes_(y = ~ target.lab,
                     label = ~ lab.format(target.lab, decimals, y.percent)),
                na.rm = TRUE,
                size = lab.size,
                hjust = lab.just,
                family = text.family) +
      geom_text_interactive(aes_(y = ~ lcl.lab,
                     label = ~ lab.format(lcl.lab, decimals, y.percent)),
                na.rm = TRUE,
                size = lab.size,
                hjust = lab.just,
                family = text.family) +
      geom_text_interactive(aes_(y = ~ ucl.lab,
                     label = ~ lab.format(ucl.lab, decimals, y.percent)),
                na.rm = TRUE,
                size = lab.size,
                hjust = lab.just,
                family = text.family) +
      geom_text_interactive(aes_(y = ~ cl.lab,
  ...
  if (is.finite(freeze)) {
    p <- p +
      geom_vline_interactive(aes_(xintercept = freeze), colour = col1, linetype = 5)
  }
  ...
  if (nrow(x.notes)) {
    p <- p +
      geom_segment_interactive(aes_(xend = ~ x, yend = Inf),
                   data = x.notes,
                   linetype = 3,
                   colour = col1) +
